...
use crate::h3::frame::{H3Frame, decode_h3_frame, encode_h3_frame};
use crate::h3::qpack::{QpackDecoder, QpackEncoder};
use crate::h3::{
    H3_STREAM_TYPE_CONTROL, H3_STREAM_TYPE_QPACK_DECODER, H3_STREAM_TYPE_QPACK_ENCODER, H3Settings,
};
...
pub(crate) struct RequestStreamState<const H3_HDR_BUF: usize = 512, const H3_DATA_BUF: usize = 1024>
{
    pub stream_id: u64,
    /// Buffer for received HEADERS frame data (QPACK-encoded).
    pub headers_data: Buf<H3_HDR_BUF>,
    pub headers_received: bool,
    /// Buffer for received DATA.
    pub data_buf: Buf<H3_DATA_BUF>,
    pub data_available: bool,
    pub fin_received: bool,
}

impl<const H3_HDR_BUF: usize, const H3_DATA_BUF: usize>
    RequestStreamState<H3_HDR_BUF, H3_DATA_BUF>
{
...
> {
    ...
    pub(crate) sio_bufs:
        crate::connection::io::QuicStreamIoBufs<MAX_STREAMS, STREAM_BUF, SEND_QUEUE>,
    ...
    #[cfg(not(feature = "alloc"))]
    pub(crate) request_streams: heapless::Vec<RequestStreamState<H3_HDR_BUF, H3_DATA_BUF>, 8>,
    #[cfg(feature = "alloc")]
    pub(crate) request_streams: alloc::vec::Vec<RequestStreamState<H3_HDR_BUF, H3_DATA_BUF>>,
    ...
    C::Hkdf: Default,
{
    ...
    fn send_stream_type(&mut self, stream_id: u64, stream_type: u64) -> Result<(), Error> {
        ...
        self.quic
            .stream_send(&mut sio, stream_id, &buf[..len], false)?;
        Ok(())
    }
    ...
    pub(crate) fn process_quic_events(
        &mut self,
        is_server: bool,
        scratch: &mut [u8],
    ) -> Result<(), Error> {
        ...
        for ev in quic_events {
            match ev {
                ...
                Event::StreamOpened(stream_id) | Event::StreamReadable(stream_id) => {
                    self.handle_stream_data(stream_id, is_server, scratch)?;
                }
                ...
                Event::StreamReset {
                    ...
                    self.push_h3_event(H3Event::StreamReset {
                        stream_id,
                        error_code,
                    });
                }
    ...
    fn handle_stream_data(
        ...
        if crate::transport::stream::is_unidirectional(stream_id) {
            self.handle_uni_stream_data(stream_id, scratch)?;
        } else {
            self.handle_request_stream_data(stream_id, is_server, scratch)?;
        }
        Ok(())
    }
    ...
    fn handle_uni_stream_data(&mut self, stream_id: u64, scratch: &mut [u8]) -> Result<(), Error> {
            ...
            || Some(stream_id) == self.peer_decoder_stream
        {
            ...
            // frames (e.g. GOAWAY).
            if Some(stream_id) == self.peer_control_stream {
                self.read_control_stream_frames(stream_id, scratch)?;
            }
        ...
        let mut sio = self.sio_bufs.as_io();
        let (read_len, fin) = match self.quic.stream_recv(&mut sio, stream_id, scratch) {
            Ok(r) => r,
        ...
        let (stream_type, type_len) = crate::varint::decode_varint(&scratch[..read_len])?;
        ...
        match stream_type {
            H3_STREAM_TYPE_CONTROL => {
                ...
                let remaining = &scratch[type_len..read_len];
    ...
    fn read_control_stream_frames(
        &mut self,
        stream_id: u64,
        scratch: &mut [u8],
    ) -> Result<(), Error> {
        let mut sio = self.sio_bufs.as_io();
        let (read_len, _fin) = match self.quic.stream_recv(&mut sio, stream_id, scratch) {
            Ok(r) => r,
            Err(Error::WouldBlock) => return Ok(()),
            Err(e) => return Err(e),
        };
        ...
        if read_len > 0 {
            self.process_control_data(&scratch[..read_len])?;
        }
        Ok(())
    }
    ...
    pub(crate) fn handle_request_stream_data(
        ...
        if !exists {
            ...
            let _ = self
                .request_streams
                .push(RequestStreamState::new(stream_id));
        }
...
mod tests {
    ...
    fn full_http3_request_response() {
        ...
        server.send_body(header_stream_id, body, true).unwrap();
    ...
    fn request_stream_state_new() {
        let rs: RequestStreamState<512, 1024> = RequestStreamState::new(42);
    ...
    fn client_poll_event_empty_initially() {
        ...
        let mut scratch = [0u8; 2048];
        let _ev = client.poll_event(&mut scratch);
    }
    ...
    fn server_poll_event_empty_initially() {
        ...
        let mut scratch = [0u8; 2048];
        let _ev = server.poll_event(&mut scratch);
    }
